...
│       └─→ AWS ALB (Standalone, managed by Terraform)                        │
│            └─→ Target Group (IP targets)                                    │
│                 └─→ WordPress Pods (via TargetGroupBinding)                 │
...
| **Load Balancing** | Standalone ALB, target groups, listeners, security groups, WAF, Route53 | `modules/standalone-alb`, `modules/waf-regional` (`stacks/infra`) |
...
| **Application Add-ons** | External Secrets Operator, AWS Load Balancer Controller (TargetGroupBinding), Karpenter, observability stack | `modules/secrets-operator`, `modules/edge-ingress`, `modules/karpenter`, `modules/observability` (`stacks/app`) |
| **Application** | Bitnami WordPress Helm release with TargetGroupBinding | `modules/app-wordpress` (`stacks/app`) |
...
- Bitnami Helm release configured to:
  ...
  - Use ClusterIP service (no Ingress resources created).
  - Configure HPA parameters (CPU/memory targets).
  - Optionally bootstrap admin credentials via ESO-managed secret (`wp-admin`).
- TargetGroupBinding resource:
  - References the WordPress ClusterIP service.
  - Automatically registers/deregisters pod IPs with the ALB target group.
  - Handles pod scaling events (HPA, Karpenter node scaling).
  - Manages target health based on Kubernetes readiness probes.
- Relies on remote state outputs: cluster metadata, secrets ARNs, database endpoint, target group ARN.

### TargetGroupBinding Architecture

The TargetGroupBinding provides the bridge between the standalone ALB (managed by Terraform) and WordPress pods (managed by Kubernetes):
...
spec:
  serviceRef:
    name: wordpress
    port: 80
  targetGroupARN: <target-group-arn-from-infra-stack>
  targetType: ip
  networking:
    ingress:
      - from:
          - securityGroup:
              groupID: <alb-security-group-id>
        ports:
          - protocol: TCP
            port: 8080
```
...
**Key Benefits**:
- **Automatic Registration**: Pod IPs are automatically registered/deregistered as pods scale
- **Karpenter Integration**: Works seamlessly with dynamic node provisioning
...
2. **Edge Controls**:
   ...
   - AWS WAF inspects traffic with managed rule sets (rate limiting, XML-RPC blocking, OWASP protection).
3. **Load Balancing**: Standalone ALB routes requests to target group containing WordPress pod IPs.
4. **Target Group**: ALB forwards traffic directly to healthy WordPress pod IPs (registered via TargetGroupBinding).
5. **Application Pod**:
...
6. **Responses**: Pod returns HTML/PHP response directly through ALB → client. Static assets may be served directly from EFS; optional S3 offload can be enabled in WordPress.
